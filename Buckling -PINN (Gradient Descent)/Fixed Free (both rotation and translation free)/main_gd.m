...
for iter = 1:numIterations
    ...
    [params, averageGrad, averageSqGrad] = adamupdate(params, grads, averageGrad, averageSqGrad, iter, 6*1e-3);
...
function [loss, gradW_1, gradb, gradw_2, grad_lambda] = modelLoss(W_1,b,w_2,lambda ,x_colloc , m2 ,n, L)
    ...
    % Boundary conditions for fixed-free beam
    x_bc = zeros(3, n);
    y_bc = zeros(3, 1);
    x_bc(1, :) = cos(W_1 * 0 + b)';                                       % w(0)
    x_bc(2, :) = -cos(W_1 * L + b)' .* (W_1 .^ 2)';                        % w''(L)
    x_bc(3, :) = -sin(W_1 * 0 + b)' .* W_1';                               % w'(0)
