...
m2 = 50;              % Number of collocation points
delta = 0.01;
x_colloc = linspace(delta, L - delta, m2);
theoriticalEigenVal = 39.47;   

% Initial guess
lambda = dlarray(1);  % Start with lambda = 1
...
for iter = 1:numIterations
    ...
    [params, averageGrad, averageSqGrad] = adamupdate(params, grads, averageGrad, averageSqGrad, iter, 1e-2);
    ...
    if mod(iter, 1) == 0 || iter == numIterations
...
function [loss, gradW_1, gradb, gradw_2, grad_lambda] = modelLoss(W_1,b,w_2,lambda ,x_colloc , m2 ,n, L)
    ...
    x_bc(1, :) = cos(W_1 * 0 + b)';
    x_bc(2, :) = cos(W_1 * L + b)';
    x_bc(3, :) = -(sin(W_1 * 0 + b) .* W_1)';
    x_bc(4, :) = -(sin(W_1 * L + b) .* W_1)';

    % Reference point to avoid trivial solution
    xd = 0.5;
    Xref = cos(W_1 * xd + b)';
