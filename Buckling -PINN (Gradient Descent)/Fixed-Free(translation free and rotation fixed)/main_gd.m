...
for iter = 1:numIterations
    ...
    [params, averageGrad, averageSqGrad] = adamupdate(params, grads, averageGrad, averageSqGrad, iter,6e-3);
...
grid on; box on;
exportgraphics(fig1,'loss_history.eps','ContentType','vector');  % EPS
...
grid on; box on;
exportgraphics(fig2,'eigenvalue_convergence.eps','ContentType','vector');  % EPS
...
grid on; box on;
exportgraphics(fig3,'mode_shape.eps','ContentType','vector');  % EPS
...
function [loss, gradW_1, gradb, gradw_2, grad_lambda] = modelLoss(W_1,b,w_2,lambda ,x_colloc , m2 ,n, L)
    ...
    res(end+1,:) =(sin(W_1 * L + b) .* (W_1 .^ 3))' + ...
        lambda * (-sin(W_1 * L + b) .* W_1)';          % w'''(L) + λ² w'(L)
    
    % === Boundary Conditions for Fixed-free (rotational fixed - translational free) Beam ===
    x_bc = zeros(3, n);
    y_bc = zeros(3, 1);
    x_bc(1, :) = cos(W_1 * 0 + b)';                            % w(0) = 0
    x_bc(2, :) = (-sin(W_1 * 0 + b) .* W_1)';                  % w'(0) = 0
    x_bc(3, :) = (-sin(W_1 * L + b) .* W_1)';                  % w'(L) = 0
