function [eigenVals, weights] = getEigenValWeights(eigen_guesses, x_vec, prevPop, ...
    ...
    for g = 1:numGuesses
        ...
        for iter = 1:MaxIter
            ...
            for j = 1:size(x_amp, 1) 
                ...
                Xc = [res;  sqrt(1000)*x_bc;1e1*x_amp(j, :) ;1e5* x_ortho];
                Yc = [zeros(m2, 1); sqrt(1000)*y_bc; 1e1* y_amp(j) ; 1e5*y_ortho];  
                X = ((Xc') * Xc) ;
                wopt = X\((Xc') * Yc);
                %  current loss
