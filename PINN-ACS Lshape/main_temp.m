...
for gen = 1:generations
    ...
    [lambda_arr, weight_arr, currGenLossConv, CurrGenEigenValConv] = getEigenValWeight_temp( ...
        init_guess_eigen, x_vec, prevPop, A, B, w1, b1, ...
        x_bc, y_bc, x_amp, y_amp, MaxIter, m2);
    ...
    init_guess_eigen = 50 * rand(1, guesses);  % New guesses each generation
end
...
fd_modes = getFd_modes(final_weights, w1, b1, final_lambdas, loss_convergence, eigen_convergence);


% %  funciton computes the eigen modes through finite difference method 
function fd_modes = getFd_modes(final_weights, w1, b1, final_lambdas,loss_convergence, eigen_convergence)
    ...
    numModes = 7; % lets just say we only focus on first 7 modes 
    ...
    plot_temp(final_lambdas, final_weights, w1, b1, fd_modes, X, Y, domain, loss_convergence, eigen_convergence);
end
